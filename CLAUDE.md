...
- **`EppClient`** is the single public API bean (`BEAN_EPP_CLIENT = "eppClient"`), with one method per EPP command (`checkDomains`, `createDomain`, `getHostInfo`, `updateContact`, ...). Every method wraps its typed request in `EppCommandRequest.request(request, clTrId)` and delegates to `EppGateway.sendCommand`. It also owns naive session management: a 13-minute in-memory expiry (`sessionExpiresAt`), a cached `Greeting` from `hello()`, and auto-(re)login via `LoginRequest` whenever the session looks stale — callers never call `login()`/`logout()` directly in normal use. Session-cookie persistence is delegated to `EppSessionCookieStore` rather than a shared header map.
